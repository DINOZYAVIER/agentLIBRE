...
mod tests {
    ...
    use agl_core::Fsm;
    use agl_core::agent::{
        AbsolutePath, AgentDefinitionRef, AgentFsm, AgentFsmInput, AgentFsmState,
        AgentOperationFsm, AgentOperationFsmInput, AgentOperationResult, AgentRunLimits,
        AgentRunOrigin, AuthorityGrantSet, InferenceRealizationRef, InstructionSet,
        ModelDefinitionRef, ModelFinishReason, ModelGenerationOutput, ModelGenerationResult,
        ModelSelection, ModelUsage, PackageDigest, RelativePath, WorkspaceScope,
    };
    ...
    fn accepted_content_is_stored_once_and_reconstructed_for_the_operation() {
        let root =
            std::env::temp_dir().join(format!("agl-daemon-content-{}", uuid::Uuid::now_v7()));
        let store = StoreHandle::open_at(&root).unwrap();
        ...
        let AgentRunAdmission::Created(run) =
            store.admit_agent_run(&spec, snapshot(&root)).unwrap()
        else {
            panic!("first admission must create")
        };
        let fsm = AgentFsm::new(
    ...
    fn reasoning_override_is_run_local_validated_and_immutable_on_replay() {
        ...
        let AgentRunAdmission::Created(run) =
            store.admit_agent_run(&spec, configured.clone()).unwrap()
        else {
    ...
    fn absent_aggregate_input_limit_round_trips_as_null_not_a_numeric_sentinel() {
        let root =
            std::env::temp_dir().join(format!("agl-unlimited-input-{}", uuid::Uuid::now_v7()));
        let store = StoreHandle::open_at(&root).unwrap();
        let conversation_id = ConversationId::generate();
        let mut configured = snapshot(&root);
        ...
        let AgentRunAdmission::Created(run) = store.admit_agent_run(&spec, configured).unwrap()
        else {
            panic!("expected creation")
        };
    ...
    fn independent_conversation_admission_is_exclusive_but_replay_is_allowed() {
        let root =
            std::env::temp_dir().join(format!("agl-conversation-busy-{}", uuid::Uuid::now_v7()));
        let store = StoreHandle::open_at(&root).unwrap();
        let conversation_id = ConversationId::generate();
        bind_conversation(&store, &root, conversation_id);
        let barrier = Arc::new(Barrier::new(8));
    ...
    fn pending_memory_round_trips_by_workspace_root() {
        let root =
            std::env::temp_dir().join(format!("agl-pending-memory-{}", uuid::Uuid::now_v7()));
        std::fs::create_dir_all(&root).unwrap();
        let store = StoreHandle::open_at(&root).unwrap();
