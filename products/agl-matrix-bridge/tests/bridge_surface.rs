...
#[test]
fn check_config_requires_persistent_thread_state() {
    let temp = TempDir::new("missing-state-path");
    let config = temp.write(
        "bridge.toml",
        r#"
[matrix]
...
"#,
    ...
    assert_failure(&output);
    assert_contains(&stderr(&output), "bindings.path is required");
}
...
#[test]
fn public_cli_has_no_synthetic_event_or_room_command_surface() {
    let output = run_bridge(&["--help"]);

    assert_success(&output);
    let stdout = stdout(&output);
    assert!(!stdout.contains("handle-test-event"));
    assert!(!stdout.contains("send"));
    assert!(!stdout.contains("bind"));
}

#[test]
fn sync_requires_durable_store_before_network_and_exits_permanently() {
    let temp = TempDir::new("sync-missing-store");
    let config = temp.write(
        "bridge.toml",
        r#"
[matrix]
...
[bindings]
path = "/tmp/agl-matrix-bridge-test-state.json"
"#,
    );
    let output = run_bridge(&["sync", "--config", &config.display().to_string()]);
    assert_eq!(output.status.code(), Some(78));
    assert_contains(&stderr(&output), "matrix.store_path is required");
    assert!(!stderr(&output).contains("fixture-not-a-real-token"));
}
...
impl TempDir {
    ...
    fn write(&self, name: &str, content: &str) -> PathBuf {
        ...
        let content = if name == "bridge.toml" {
            if content.contains("[agl]") {
                content.replacen(
                    "[agl]",
                    "[agl]\nfunction_path = \"/tmp/functions/matrix\"\nworkspace_path = \"/tmp/workspace\"",
                    1,
                )
            } else {
                format!(
