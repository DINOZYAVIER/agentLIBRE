...
use agl_matrix_bridge::{BridgeApp, BridgeConfig};
#[cfg(unix)]
use agl_matrix_bridge::{
    LazyDaemonClient, MatrixDeviceVerificationRequest, MatrixPasswordLogin, MatrixRuntime,
    MatrixSasPresentation, MatrixUserDevice,
};
...
async fn main() {
    ...
    if let Err(err) = run(Cli::parse()).await {
        eprintln!("error: {err:#}");
        #[cfg(unix)]
        if agl_matrix_bridge::is_permanent_matrix_error(&err) {
            std::process::exit(78);
        }
        std::process::exit(1);
    }
...
fn check_config(path: PathBuf) -> Result<()> {
    ...
    let app = BridgeApp::from_config(config.clone())?;
    let state = app.state();
    ...
    println!("replay_entries={}", state.replay.len());
    Ok(())
}
...
fn status(path: PathBuf, socket: Option<PathBuf>) -> Result<()> {
    ...
    let client = LazyDaemonClient::new(socket_path);
...
async fn sync(path: PathBuf, socket: Option<PathBuf>) -> Result<()> {
    let mut config = BridgeConfig::load(&path).context(agl_matrix_bridge::PermanentMatrixFault)?;
    config
        .validate()
        .map_err(|err| anyhow::anyhow!("bridge config is invalid: {err:?}"))
        .context(agl_matrix_bridge::PermanentMatrixFault)?;
