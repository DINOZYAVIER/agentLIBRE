...
impl MatrixRuntime {
    pub async fn from_config(config: BridgeConfig, socket_path: PathBuf) -> Result<Self> {
        config
            ...
            .context(crate::PermanentMatrixFault)?;
        let session =
            matrix_session_from_config(&config.matrix).context(crate::PermanentMatrixFault)?;
        matrix_store_path(&config.matrix)
            .context("matrix.store_path is required to persist the accepted sync position")
            .context(crate::PermanentMatrixFault)?;
        let bot_user_id = config.matrix.user_id.clone();
        let app = Arc::new(Mutex::new(BridgeApp::from_config(config.clone())?));
    ...
    pub async fn sync_forever(&self) -> Result<()> {
        ...
        crate::sync_retry::sync(&self.client).await
    }
                ...
                async move {
                    ...
                    match handle_inbound_on_blocking_thread(app, socket_path, inbound).await {
                        ...
                        Err(error) => {
                            ...
                            let _ =
                                send_matrix_notice(&room, &reply_context, "Agent request failed.")
                                    .await;
                        }
                    }
                }
...
fn inbound_event_from_original(
    ...
    let thread_root_event_id = thread_root_event_id(&event.content)
...
async fn send_matrix_text(room: &Room, context: &MatrixReplyContext, body: &str) -> Result<()> {
    ...
    room.send(content)
        .with_transaction_id(stable_event_transaction_id(&context.reply_event_id, "reply").into())
        .await
...
async fn send_matrix_notice(room: &Room, context: &MatrixReplyContext, body: &str) -> Result<()> {
    ...
    content.relates_to = Some(message_relation(context)?);
    room.send(content)
        .with_transaction_id(stable_event_transaction_id(&context.reply_event_id, "notice").into())
        .await
        .context("Matrix room send failed")?;
    Ok(())
}

fn stable_event_transaction_id(event_id: &str, action: &str) -> String {
    let mut hasher = Sha256::new();
    for field in [event_id.as_bytes(), action.as_bytes()] {
