...
#[test]
fn root_room_event_starts_a_thread() {
    let context = MatrixReplyContext {
        thread_root_event_id: "$event:example".to_string(),
        reply_event_id: "$event:example".to_string(),
    };

    match message_relation(&context).unwrap() {
        Relation::Thread(thread) => {
            assert_eq!(thread.event_id.as_str(), "$event:example");
            assert_eq!(
...
fn message_relation_replies_inside_existing_thread() {
    let context = MatrixReplyContext {
        thread_root_event_id: "$thread:example".to_string(),
        reply_event_id: "$event:example".to_string(),
    };
