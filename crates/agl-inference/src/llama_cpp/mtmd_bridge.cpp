...
extern "C" int32_t agl_mtmd_eval_images(
        ...
        char * err,
        size_t err_len) try {
    if (context == nullptr || llama_context == nullptr || prompt == nullptr ||
        image_data == nullptr || image_lengths == nullptr || image_count == 0 ||
        batch_size <= 0 || out_positions == nullptr || out_tokens == nullptr) {
    ...
    for (size_t index = 0; index < image_count; ++index) {
        ...
        mtmd_helper_bitmap_wrapper wrapper = mtmd_helper_bitmap_init_from_buf(
            context, image_data[index], image_lengths[index], false);
    ...
    const int32_t tokenize_result = mtmd_tokenize(
        context, chunks.get(), &input, bitmaps.data(), bitmaps.size());
