...
impl EffectDeclaration {
    ...
    pub fn for_standard(id: EffectId) -> Option<Self> {
        let authority_class = match id.as_str() {
            "agl:host.screen_capture" => AuthorityClass::HostObservation,
            "agl:agent.spawn" => AuthorityClass::AgentDelegation,
            "agl:session.working_directory" => AuthorityClass::SessionMutation,
            "agl:process.spawn" => AuthorityClass::ProcessSpawn,
            "agl:process.control" => AuthorityClass::ProcessControl,
            "agl:process.host_execution" => AuthorityClass::HostExecution,
            "agl:process.shell_login_startup" => AuthorityClass::ShellStartup,
            "agl:repo.files" | "agl:repo.workspace" => AuthorityClass::RepositoryMutation,
            "agl:repo.hooks" => AuthorityClass::RepositoryHooks,
            ...
            | "agl:store.cron"
            | "agl:store.schema"
            | "agl:store.idempotency" => AuthorityClass::DurableStoreMutation,
            "agl:matrix.outbox" => AuthorityClass::ExternalDelivery,
            "agl:store.permission_requests" | "agl:store.permission_grants" => {
                AuthorityClass::PermissionMutation
            }
            "agl:skill.trust" => AuthorityClass::TrustMutation,
            _ => return None,
        };
        Some(Self::new(id, authority_class.as_str()))
    }
}
...
impl HookDeclaration {
    pub fn new(id: HookId, event: HookEvent) -> Self {
        Self {
            id,
            event,
            input_schema: generic_object_schema(),
            output_schema: generic_object_schema(),
        }
    }
...
impl ExtensionDescriptor {
    ...
    pub fn with_effects(mut self, effects: impl IntoIterator<Item = EffectDeclaration>) -> Self {
    ...
    pub fn validate(&self) -> Result<(), DeclarationError> {
        ...
        if self.id.as_str() == "core" && self.source != ExtensionSource::Builtin {
            return Err(DeclarationError::ReservedExtensionNamespace {
                extension_id: self.id.clone(),
            });
        }
        ...
        for hook in &self.hooks {
            ...
            if hook.id.extension_namespace() != self.id.as_str() {
                return Err(DeclarationError::HookExtensionMismatch {
        ...
        for tool in &self.tools {
            tool.validate()?;
            if tool.id.extension_namespace() != self.id.as_str() {
                return Err(DeclarationError::ToolExtensionMismatch {
                    tool_id: tool.id.clone(),
                    extension_id: self.id.clone(),
                });
            }
...
pub enum DeclarationError {
    ...
    HookExtensionMismatch {
        hook_id: HookId,
        extension_id: ExtensionId,
    },
    ToolExtensionMismatch {
        tool_id: ToolId,
        extension_id: ExtensionId,
    },
    UndeclaredEffect {
...
impl Display for DeclarationError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::HookExtensionMismatch {
                hook_id,
                extension_id,
            } => write!(
                formatter,
                "hook `{hook_id}` namespace must match extension `{extension_id}`"
            ),
            Self::ToolExtensionMismatch {
                tool_id,
                extension_id,
            } => write!(
                formatter,
                "tool `{tool_id}` namespace must match extension `{extension_id}`"
            ),
