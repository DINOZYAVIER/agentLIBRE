...
static TERMINAL_ACCESS: OnceLock<TerminalAccess> = OnceLock::new();
...
impl TerminalAccess {
    fn from_runtime(runtime: &UiRuntimeConfig) -> Result<Self> {
    ...
    fn client(&self) -> Result<TerminalClient<UnixTerminalTransport>> {
...
fn terminal_access() -> Result<&'static TerminalAccess> {
    TERMINAL_ACCESS
        .get()
...
struct AttachmentStarted {
    writable: bool,
}

struct ExecutionAttachment {
    access: TerminalAccess,
    terminal_id: agl_terminal::TerminalId,
    stream_id: agl_terminal::TerminalStreamId,
    next_sequence: u64,
...
#[derive(Debug)]
enum ExecutionAttachmentEvent {
    Output(AttachmentOutputEvent),
    Finished(AttachmentFinishedEvent),
}

#[derive(Debug)]
struct AttachmentOutputEvent {
    chunk: agl_exec::ExecutionOutputChunk,
}

#[derive(Debug)]
struct AttachmentFinishedEvent {
    state: agl_exec::ExecutionState,
    last_delivered_sequence: u64,
}

impl ExecutionAttachment {
    async fn attach(terminal_id: &TerminalId, after_sequence: u64, writable: bool) -> Result<Self> {
    ...
    async fn input(&self, bytes: agl_exec::ProcessBytes, _eof: bool) -> Result<()> {
        self.access
            .client()?
            .input(
    ...
    async fn resize(&self, columns: u16, rows: u16) -> Result<()> {
        self.access
            .client()?
            .resize(
                self.terminal_id.clone(),
...
struct InteractiveState {
    ...
    human_commands: Vec<LocalHumanCommandCard>,
    no_color: bool,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum LocalHumanCommandState {
    Running,
    Completed,
    OutcomeUnknown,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
struct LocalHumanCommandCard {
    terminal_id: TerminalId,
    command_sequence: u64,
    command: String,
    state: LocalHumanCommandState,
}
...
struct ShellSubmissionCompletion {
    ...
    outcome: std::result::Result<ShellCommandAccepted, ShellSubmissionFailure>,
}

struct ShellCommandAccepted {
    terminal_id: TerminalId,
    command_sequence: u64,
}
...
fn shell_submit_outcome_uncertain(error: &agl_terminal_client::ClientError) -> bool {
    !matches!(
        error,
...
async fn execute_shell_submission(
    ...
    let local_terminal_id = agl_terminal::TerminalId::parse(terminal.terminal_id.as_str())
        .map_err(|error| anyhow::anyhow!(error.to_string()));
    let local_topology_id = agl_exec::OpaqueOwnerId::new(task.session_id.as_str())
...
fn update_local_human_commands(state: &mut UiState, terminal: &TerminalSessionView) {
    for card in state
        .human_commands
        .iter_mut()
    ...
    impl PanicGuardParentTerminal {
        fn spawn() -> Self {
            ...
            let child = std::process::Command::new(std::env::current_exe().unwrap())
                .args([
                    "--exact",
                    "tests::panic_hook_restores_parent_terminal_in_native_pty",
                    "--nocapture",
                    "--test-threads=1",
                ])
