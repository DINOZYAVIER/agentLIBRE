...
impl SqliteExecutionRepository {
    ...
    fn from_store(store: TerminalStore, finished_retention: Duration) -> Self {
        Self {
        ...
        self.with_store(|store| {
            store.transaction(|tx| {
                ...
                ) in candidates
                {
                    ...
                    // Correlation is deliberately opaque here. The terminal
                    // owner records `outcome_unknown`; the agent owner observes
                    // that status and performs its own run/step transition.
                    let _ = (namespace, namespace_version, raw_run_id, raw_step_id);
