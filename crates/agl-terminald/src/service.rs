...
use crate::storage::{PersistedAdmission, SqliteAdmissionRepository};
...
pub struct TerminalService {
    ...
    admission_repository: Option<Arc<SqliteAdmissionRepository>>,
}
...
impl TerminalService {
    ...
    pub(crate) fn with_admission_repository(
        mut self,
        repository: Arc<SqliteAdmissionRepository>,
    ) -> Result<Self, ProcessError> {
        let mut terminal_admissions = BTreeMap::new();
        let mut execution_admissions = BTreeMap::new();
        let mut execution_fingerprints = BTreeMap::new();
        for persisted in repository.list().map_err(ProcessError::from)? {
            let authority =
                agl_exec::AuthorityFingerprint::new(persisted.authority_fingerprint.clone())
                    .map_err(|error| corrupt_admission(error.to_string()))?;
            match persisted.resource_kind.as_str() {
                "terminal" => {
                    let terminal_id = TerminalId::parse(&persisted.resource_id)
                        .map_err(|error| corrupt_admission(error.to_string()))?;
                    let operations = serde_json::from_str(&persisted.operations_json)
                        .map_err(|error| corrupt_admission(error.to_string()))?;
                    terminal_admissions.insert(
                        terminal_id,
                        AdmissionState {
                ...
                "execution" => {
                    let execution_id = agl_exec::ExecutionId::parse(&persisted.resource_id)
                        .map_err(|error| corrupt_admission(error.to_string()))?;
                    let operations = serde_json::from_str(&persisted.operations_json)
                        .map_err(|error| corrupt_admission(error.to_string()))?;
                    execution_fingerprints
                        .insert(persisted.request_fingerprint, execution_id.clone());
                    execution_admissions.insert(
                        execution_id,
                ...
                _ => return Err(corrupt_admission("unknown admission resource kind")),
            }
        }
        self.admissions = Mutex::new(terminal_admissions);
        self.execution_admissions = Mutex::new(execution_admissions);
        self.execution_fingerprints = Mutex::new(execution_fingerprints);
        self.admission_repository = Some(repository);
        Ok(self)
    }
    ...
    fn dispatch(
        ...
        request
            .expected_generation
            .require_exact(self.identity.installed_generation())
            .map_err(identity_failure)?;
        if let Some(expected_process_generation) = &request.expected_process_generation
            && expected_process_generation != self.identity.process_generation_id()
        {
            return Err(identity_failure(
                agl_terminal_protocol::TerminalGenerationError::IdentityMismatch,
            ));
        }
        ...
        match request.request {
            ...
            TerminalRequestKind::ListExecutions { filter } => {
                let authority = authority.as_ref().ok_or_else(authority_denied)?;
                ...
                let statuses = self
                    ...
                    .filter(|status| {
                        admissions
                            ...
                            .is_some_and(|admission| &admission.authority_fingerprint == authority)
                    })
                    .collect();
                Ok(TerminalResponseKind::ExecutionList { statuses })
            }
            TerminalRequestKind::RevokeGrant { grant_id } => {
                let authority = authority.as_ref().ok_or_else(authority_denied)?;
                self.require_known_authority(authority)?;
                let terminated_executions = self
                    .process
                    .terminate_grant(&grant_id)
                    .map_err(process_failure)?;
                Ok(TerminalResponseKind::GrantRevoked {
                    terminated_executions: u32::try_from(terminated_executions).unwrap_or(u32::MAX),
                })
            }
            ...
            TerminalRequestKind::ExecuteAgentCommand {
                ...
                if &admission.authority_fingerprint != authority {
                    return Err(authority_denied());
                }
                let operations = admission.operations.clone();
                let fingerprint = admission.authority_fingerprint.clone();
                let request = terminal_ensure_request(*admission)?;
                let deadline = timeout_ms
                    .map(Duration::from_millis)
                    .and_then(|duration| Instant::now().checked_add(duration));
                let result = self
                    .registry
                    .execute_agent_command(request, command, deadline)
                    .map_err(process_failure)?;
                ...
                Ok(TerminalResponseKind::AgentCommand { result })
            }
            ...
            TerminalRequestKind::Promote {
                ...
                let record = self
                    .registry
                    .promote_ephemeral_owner(&terminal_id, &topology_id, owner)
                    .map_err(process_failure)?;
            ...
            TerminalRequestKind::Retire { terminal_id } => {
                ...
                let record = self
                    .registry
                    .retire_terminal_slot(&terminal_id)
                    .map_err(process_failure)?;
                Ok(TerminalResponseKind::TerminalRecord {
                    record: Box::new(record),
                })
    ...
    fn ensure(
        ...
        let request = terminal_ensure_request(admission.clone())?;
        let record = self
            .registry
    ...
    fn persist_admission<T: serde::Serialize>(
        &self,
        resource_kind: &str,
        resource_id: &str,
        ...
        let Some(repository) = &self.admission_repository else {
            return Ok(());
        };
        let operations_json = serde_json::to_string(operations).map_err(|_| internal_failure())?;
        repository
            .record(&PersistedAdmission {
                resource_kind: resource_kind.to_owned(),
                resource_id: resource_id.to_owned(),
                request_fingerprint: request_fingerprint.to_owned(),
                authority_fingerprint: authority_fingerprint.as_str().to_owned(),
                operations_json,
            })
            .map_err(|_| internal_failure())
    }

    fn require_known_authority(
        &self,
        ...
        let terminal_known = self
            .lock_admissions()?
            .values()
            .any(|admission| &admission.authority_fingerprint == authority);
        let execution_known = self
            .lock_execution_admissions()?
            .values()
            .any(|admission| &admission.authority_fingerprint == authority);
        if terminal_known || execution_known {
            Ok(())
        } else {
    ...
    fn descriptor(
        ...
        Ok(TerminalDescriptor {
            ...
            service_generation: self.identity.process_generation_id().clone(),
            state: record.state,
...
pub async fn serve_unix(
    ...
    serve_unix_listener(service, listener, cancellation).await
}
...
fn terminal_ensure_request(
    admission: TerminalAdmission,
) -> std::result::Result<TerminalEnsureRequest, TerminalFailure> {
    let history_seed =
        TerminalHistorySeed::from_commands(admission.history_seed).map_err(process_failure)?;
...
fn identity_failure<E>(_error: E) -> TerminalFailure {
    TerminalFailure {
...
fn corrupt_admission(message: impl Into<String>) -> ProcessError {
    ProcessError::new(
        ProcessErrorCode::StoreCorrupt,
        format!(
            "terminal service admission store is corrupt: {}",
            message.into()
        ),
    )
}
