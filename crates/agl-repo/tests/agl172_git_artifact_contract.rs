...
fn commit_request_rejects_empty_duplicate_directory_missing_and_unchanged_entries() {
    ...
    let repository = ArtifactGitRepository::open(&fixture.parent).unwrap();
    let binding = repository.verify_binding(&declaration()).unwrap();
    let mut operations = MemoryArtifactCommitRepository::default();
    ...
    for entries in [
        ...
        assert!(
            repository
                .commit_artifact(&binding, request(entries), &mut operations)
                .is_err()
        );
    }
}
...
fn replay_with_different_commit_input_is_a_conflict() {
    ...
    repository
        .commit_artifact(&binding, first, &mut operations)
        .unwrap();
    let error = repository
        .commit_artifact(&binding, changed, &mut operations)
        .unwrap_err();
...
fn every_ref_update_failpoint_recovers_without_duplicate_commits() {
    for failpoint in [
        ...
        let interrupted = repository.commit_artifact_with_failpoint(
            &binding,
            request,
            &mut operations,
            failpoint,
        ...
        let recovered = repository.recover_incomplete(&mut operations).unwrap();
        ...
        assert!(
            repository
                .recover_incomplete(&mut operations)
                .unwrap()
                .is_empty()
        );
        assert_eq!(
...
fn independent_parent_gitlink_change_becomes_immutable_conflict() {
    ...
    let result = repository.recover_incomplete(&mut operations).unwrap();
