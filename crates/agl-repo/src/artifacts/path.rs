...
pub(super) fn artifact_policy_error_blocks_writes(error: &str) -> bool {
    error.starts_with("workspace_manifest_invalid")
        ...
        || error.contains("definition_changed")
        || error.contains("storage_changed")
        || error.contains("required_changed")
        || error.contains("kind_changed")
        || error.contains("access_changed")
        || error.contains("validation_changed")
        || error.contains("url_changed")
        || error.contains("rev_changed")
        || error.contains("commit_changed")
        || error.contains("tree_changed")
}
