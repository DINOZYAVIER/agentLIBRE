...
pub const MAX_AGENT_TERMINAL_COMMAND_BYTES: usize = 32 * 1024;
...
impl QueuedTerminalCommand {
    ...
    pub fn submission(&self) -> ProcessBytes {
        let mut bytes = Vec::with_capacity(self.command.len() + 1);
        bytes.extend_from_slice(self.command.as_bytes());
        bytes.push(b'\n');
        ProcessBytes::from_bytes(&bytes)
    }
}
...
impl AgentTerminalCommandQueue {
    ...
    pub fn enqueue(&mut self, command: String, deadline: Option<Instant>) -> Result<u64> {
        if command.is_empty()
            || command.len() > MAX_AGENT_TERMINAL_COMMAND_BYTES
            || command.contains('\0')
        {
            return Err(ProcessError::new(
                ProcessErrorCode::InvalidRequest,
                "agent terminal command must be nonempty, bounded, and contain no NUL",
            ));
        }
    ...
    pub fn mark_started(&mut self, integration_sequence: u64) -> Result<()> {
...
mod tests {
    ...
    fn commands_are_fifo_exact_and_never_interleave() {
        ...
        assert_eq!(result.output.after_sequence, 8);
