    ...
    T: DaemonTransport,
{
    ...
    fn send(&mut self, kind: DaemonRequestKind) -> Result<String, ClientError> {
        let request_id = format!("req-{:06}", self.next_request_index);
    ...
    fn read_correlated_event(&mut self, request_id: &str) -> Result<DaemonEvent, ClientError> {
...
mod tests {
    ...
    use agl_protocol::{
        ...
        SessionStatus, TurnFinishedEvent,
    };
    ...
    fn hello_writes_alpha_request_and_decodes_response() {
        ...
        assert_eq!(request.request_id, "req-000001");
