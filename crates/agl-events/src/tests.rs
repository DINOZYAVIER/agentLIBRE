...
fn example_events() -> Vec<AgentEvent> {
    ...
    vec![
        ...
        AgentEvent::HookBatchPrepared {
            turn_id: turn_id.clone(),
            event: "model.response".to_string(),
            required_hooks: vec!["guard.response_required".to_string()],
            optional_hooks: vec!["guard.response_optional".to_string()],
            required_count: 1,
            optional_count: 1,
        },
        AgentEvent::HookBatchStarted {
            ...
            event: "model.response".to_string(),
            hook_ids: vec![
                "guard.response_required".to_string(),
                "guard.response_optional".to_string(),
            ],
            required_count: 1,
            optional_count: 1,
        },
        AgentEvent::HookBatchFinished {
            turn_id: turn_id.clone(),
            event: "model.response".to_string(),
            outcome: HookBatchOutcomeEvent::Warn,
            required_count: 1,
            optional_count: 1,
            ...
            message_codes: vec!["response.warning".to_string()],
            results: vec![HookResultEvent {
                hook_id: "guard.response_optional".to_string(),
                status: HookBatchOutcomeEvent::Warn,
                message_codes: vec!["response.warning".to_string()],
            }],
            duration_ms: Some(5),
        },
        AgentEvent::HookBatchBlocked {
            turn_id: turn_id.clone(),
            event: "turn.finish".to_string(),
            outcome: HookBatchOutcomeEvent::Fail,
            message: "blocked because answer contained secret hook detail".to_string(),
            failed_required_count: 1,
            missing_required_hooks: Vec::new(),
            message_codes: vec!["answer.blocked".to_string()],
        },
        AgentEvent::HookRepairPrepared {
            turn_id: turn_id.clone(),
            event: "model.response".to_string(),
            attempt: 1,
            hook_ids: vec!["guard.response_required".to_string()],
            message_codes: vec!["response.repair".to_string()],
        },
...
fn safe_jsonl_omits_content_bearing_fields() {
    let forbidden = [
        ...
        "blocked because answer contained secret hook detail",
        "{bad",
