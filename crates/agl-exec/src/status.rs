...
use crate::{
    ExecutionId, ExecutionRequestId, ProcessBytes, ProcessError, ProcessErrorCode, Result,
    WriterLeaseId,
};
...
impl ExecutionPrivateCommand {
    pub fn from_argv(program: &Path, args: &[String], maximum_bytes: usize) -> Result<Self> {
        if maximum_bytes < 4 {
