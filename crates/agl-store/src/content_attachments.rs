...
pub(crate) enum ContentAttachmentWriteFailpoint {
    BeforeBlobWrite,
    AfterBlobWrite,
...
impl AglStore {
    ...
    pub fn write_content_attachment(
        &self,
        run_id: &RunId,
    ...
    ) -> Result<StoredContentAttachment> {
        self.write_content_attachment_inner(
            run_id,
            media_type,
            bytes,
            image,
    ...
    #[cfg(test)]
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn write_content_attachment_injected(
        &self,
        run_id: &RunId,
        media_type: MediaType,
        bytes: &[u8],
        ...
        source: ArtifactSource,
        retention: ArtifactRetention,
        failpoint: ContentAttachmentWriteFailpoint,
    ) -> Result<StoredContentAttachment> {
        self.write_content_attachment_inner(
            run_id,
            media_type,
    ...
    #[allow(clippy::too_many_arguments)]
    fn write_content_attachment_inner(
        &self,
        run_id: &RunId,
        media_type: MediaType,
        bytes: &[u8],
        ...
        source: ArtifactSource,
        retention: ArtifactRetention,
        failpoint: Option<ContentAttachmentWriteFailpoint>,
    ) -> Result<StoredContentAttachment> {
        if bytes.is_empty() {
            return Err(StoreError::InvalidValue {
                field: "artifact.bytes",
                value: "0".to_string(),
                reason: "content attachment bytes cannot be empty",
            });
        }
        let content_attachment_id = ContentAttachmentId::generate();
        ...
        let byte_length = u64::try_from(bytes.len()).map_err(|_| StoreError::InvalidValue {
            ...
            reason: "content attachment byte length exceeds u64",
        })?;
        ...
        inject_artifact_failure(failpoint, ContentAttachmentWriteFailpoint::BeforeBlobWrite)?;
        self.persist_blob(&digest, bytes)?;
        inject_artifact_failure(failpoint, ContentAttachmentWriteFailpoint::AfterBlobWrite)?;
        let now_ms = unix_millis();
        inject_artifact_failure(
            failpoint,
        ...
        self.transaction(|transaction| {
            ...
            if stored != (media_type.mime().to_string(), byte_length) {
                return Err(StoreError::ContentAttachmentIntegrityFailed {
                    content_attachment_id: reference.content_attachment_id.to_string(),
            ...
            transaction.execute(
                ...
                params![
                    reference.content_attachment_id.as_str(),
                    digest.as_str(),
                    run_id.as_str(),
                    media_type.mime(),
                    byte_length,
    ...
    pub fn content_attachment(
        &self,
        content_attachment_id: &ContentAttachmentId,
    ) -> Result<Option<StoredContentAttachment>> {
        self.connection()
            .query_row(
            ...
            .optional()?
            .map(|row| decode_content_attachment(content_attachment_id.clone(), row))
            .transpose()
    }

    pub fn resolve_content_attachment(
        &self,
        owner_run_id: &RunId,
        reference: &ContentAttachmentRef,
    ) -> Result<ResolvedContentAttachment> {
        let stored = self
            .content_attachment(&reference.content_attachment_id)?
            .ok_or_else(|| StoreError::ContentAttachmentUnavailable {
                content_attachment_id: reference.content_attachment_id.to_string(),
            })?;
        if &stored.run_id != owner_run_id {
            return Err(StoreError::ContentAttachmentAccessDenied);
        }
        if stored.tombstoned {
            return Err(StoreError::ContentAttachmentUnavailable {
                content_attachment_id: reference.content_attachment_id.to_string(),
            });
        }
        if &stored.reference != reference {
            return Err(StoreError::ContentAttachmentIntegrityFailed {
                content_attachment_id: reference.content_attachment_id.to_string(),
                reason: "reference metadata differs from the canonical content attachment"
                    .to_string(),
            });
        }
        let path = self.blob_path(&reference.digest)?;
        let file = File::open(&path).map_err(|_| StoreError::ContentAttachmentUnavailable {
            content_attachment_id: reference.content_attachment_id.to_string(),
        })?;
        let capacity = usize::try_from(reference.byte_length).map_err(|_| {
            StoreError::ContentAttachmentIntegrityFailed {
                content_attachment_id: reference.content_attachment_id.to_string(),
                reason: "content attachment length exceeds process address space".to_string(),
            }
        })?;
        ...
        Ok(ResolvedContentAttachment {
            reference: reference.clone(),
            bytes,
        })
    }

    pub fn tombstone_run_content_attachments(&self, run_id: &RunId) -> Result<usize> {
        let now_ms = unix_millis();
        Ok(self.connection().execute(
            "UPDATE content_attachments SET state = 'tombstoned', tombstoned_at_ms = ?2
             WHERE run_id = ?1 AND state = 'live' AND retention = 'run_scoped'",
            params![run_id.as_str(), now_ms],
        )?)
    }

    pub fn garbage_collect_content_attachments(&self) -> Result<ContentAttachmentGcReport> {
        let _lock = ArtifactStoreLock::acquire(self.store_root())?;
        let content_attachment_records_deleted = self.transaction(|transaction| {
            Ok(transaction.execute(
                "DELETE FROM content_attachments WHERE state = 'tombstoned'",
                [],
            )?)
        })?;
        let mut statement = self.connection().prepare(
            "SELECT digest FROM content_blobs
             WHERE NOT EXISTS (
                 SELECT 1 FROM content_attachments WHERE content_attachments.blob_digest = content_blobs.digest
             )",
        )?;
        ...
        drop(statement);
        let mut report = ContentAttachmentGcReport {
            content_attachment_records_deleted,
            ..ContentAttachmentGcReport::default()
        };
    ...
    fn persist_blob(&self, digest: &BlobDigest, bytes: &[u8]) -> Result<()> {
        ...
        let temp = temp_root.join(ContentAttachmentId::generate().as_str());
...
impl ArtifactStoreLock {
    fn acquire(store_root: &Path) -> Result<Self> {
        ...
        let path = blob_root.join(".content-attachment.lock");
        let mut options = OpenOptions::new();
...
fn inject_artifact_failure(
    configured: Option<ContentAttachmentWriteFailpoint>,
    current: ContentAttachmentWriteFailpoint,
) -> Result<()> {
    if configured == Some(current) {
        return Err(StoreError::InvalidValue {
            field: "content_attachment.failpoint",
            value: format!("{current:?}"),
            reason: "injected content attachment write failure",
        });
    }
    Ok(())
}
...
fn decode_content_attachment(
    ...
    let image = match (row.4, row.5) {
        ...
        _ => {
            return Err(StoreError::ContentAttachmentIntegrityFailed {
                content_attachment_id: content_attachment_id.to_string(),
    ...
    Ok(StoredContentAttachment {
        run_id: RunId::parse(&row.0).map_err(|_| StoreError::ContentAttachmentIntegrityFailed {
            content_attachment_id: reference.content_attachment_id.to_string(),
...
fn parse_sensitivity(value: &str) -> Result<ArtifactSensitivity> {
    match value {
        ...
        _ => Err(StoreError::InvalidValue {
            field: "content_attachments.sensitivity",
            value: value.to_string(),
            reason: "invalid content attachment sensitivity",
        }),
    }
}
...
fn parse_retention(value: &str) -> Result<ArtifactRetention> {
    match value {
        ...
        _ => Err(StoreError::InvalidValue {
            field: "content_attachments.retention",
            value: value.to_string(),
            reason: "invalid content attachment retention",
        }),
    }
}
...
fn verify_blob_file(path: &Path, digest: &BlobDigest, expected_length: usize) -> Result<()> {
    ...
    if bytes.len() != expected_length || BlobDigest::from_bytes(&bytes) != *digest {
        return Err(StoreError::ContentAttachmentIntegrityFailed {
            content_attachment_id: "deduplicated_blob".to_string(),
