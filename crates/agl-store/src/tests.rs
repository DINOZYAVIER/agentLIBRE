...
#[test]
fn default_root_run_budget_covers_the_longest_admitted_inference_operation() {
...
fn migration_seventeen_rebuilds_live_run_table_without_losing_foreign_keys() {
    ...
    assert_eq!(report.after_schema_version, CURRENT_SCHEMA_VERSION);
    assert_eq!(report.applied_migrations.len(), 4);
...
#[test]
fn obsolete_combined_terminal_state_is_rejected_without_import() {
    let root = temp_root("obsolete-terminal-state");
    let store = AglStore::open_at(&root.path).unwrap();
    let database_path = store.database_path().to_path_buf();
    drop(store);
    let connection = rusqlite::Connection::open(&database_path).unwrap();
    connection
        .execute_batch(
            "CREATE TABLE executions(id TEXT PRIMARY KEY); INSERT INTO executions VALUES ('old');",
        )
        .unwrap();
    drop(connection);

    let error = AglStore::open_at(&root.path).unwrap_err();
    assert!(
        error
            .to_string()
            .contains("obsolete combined execution/terminal state")
    );
    let connection = rusqlite::Connection::open(database_path).unwrap();
    assert_eq!(
        connection
