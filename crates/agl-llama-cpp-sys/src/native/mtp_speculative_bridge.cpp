...
bool agl_mtp_batch_compatible(const llama_batch & batch) {
    if (batch.n_tokens <= 0 || batch.token == nullptr || batch.embd != nullptr || batch.pos == nullptr ||
        batch.n_seq_id == nullptr || batch.seq_id == nullptr) {
        return false;
    }

    for (int32_t i = 0; i < batch.n_tokens; ++i) {
        if (batch.n_seq_id[i] != 1 || batch.seq_id[i] == nullptr || batch.seq_id[i][0] != AGL_LLAMA_MTP_SEQ_ID) {
            return false;
        }
    }
...
void agl_mtp_assign_tokens(std::vector<llama_token> & dst, const llama_token * tokens, size_t count) {
    if (count == 0) {
        dst.clear();
        return;
    }
...
agl_llama_mtp_speculative * agl_llama_mtp_init(llama_context * ctx_tgt,
...
agl_llama_mtp_status agl_llama_mtp_begin(agl_llama_mtp_speculative * spec,
...
agl_llama_mtp_status agl_llama_mtp_process(agl_llama_mtp_speculative * spec, const llama_batch * batch) {
    ...
    try {
        return common_speculative_process(spec->spec, *batch) ? AGL_LLAMA_MTP_OK : AGL_LLAMA_MTP_DECODE_FAILED;
    } catch (...) {
        return AGL_LLAMA_MTP_EXCEPTION;
    }
}

agl_llama_mtp_status agl_llama_mtp_draft(agl_llama_mtp_speculative * spec,
                                         llama_pos                   n_past,
                                         llama_token                 id_last,
                                         const llama_token *         prompt_tokens,
                                         size_t                      prompt_tokens_count,
    ...
    try {
        ...
        params        = {
            true, spec->params.draft.n_max, n_past, id_last, &spec->prompt, &spec->draft,
        };
...
agl_llama_mtp_status agl_llama_mtp_accept(agl_llama_mtp_speculative * spec, uint16_t n_accepted) {
