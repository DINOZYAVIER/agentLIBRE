...
fn normalized_list_path(raw: &str) -> Result<String> {
    ...
    path.to_str()
        .map(str::to_string)
        .context("non_utf8_path: core.workspace:fs.list query path is not valid UTF-8")
}

fn validate_args(args: &ListArgs) -> Result<()> {
    ensure!(
        (1..=MAX_PAGE_SIZE).contains(&args.page_size),
        "core.workspace:fs.list page_size must be between 1 and {MAX_PAGE_SIZE}"
    );
    if let Some(cursor) = &args.cursor {
        ensure!(
            !cursor.is_empty() && cursor.len() <= MAX_CURSOR_BYTES,
            "core.workspace:fs.list cursor is invalid"
        );
        ensure!(
            cursor
                .bytes()
    ...
    if let Some(pattern) = &args.name_glob {
            ...
            "invalid_glob: core.workspace:fs.list name_glob must contain 1..={MAX_GLOB_BYTES} UTF-8 bytes"
        );
        ensure!(
            !pattern.chars().any(char::is_control),
            "invalid_glob: core.workspace:fs.list name_glob contains a control character"
        );
    }
    Ok(())
}
...
impl<'a> CheckedOutQuery<'a> {
    fn new(
        ...
        let state = {
            ...
            let state = registry.queries.get(cursor).ok_or_else(|| {
                anyhow::anyhow!(
                    "cursor_stale: core.workspace:fs.list cursor is unknown, expired, or already consumed"
                )
            })?;
            ensure!(
                state.run_id == run_id,
                "cursor_stale: core.workspace:fs.list cursor belongs to another run"
            );
            ensure!(
                &state.query == query,
                "cursor_query_mismatch: core.workspace:fs.list query fields changed while continuing"
            );
            registry
                .queries
                .remove(cursor)
                .context("core.workspace:fs.list cursor disappeared while checking it out")?
        };
        Ok(Self {
            registry,
            state: Some(state),
    ...
    fn state(&self) -> &ListQueryState {
        self.state
            .as_ref()
            .expect("checked-out core.workspace:fs.list query state is present")
    }
...
impl ListQueryRegistry {
    fn reserve_active(&mut self, run_id: &str) -> Result<()> {
        ...
        if usage.active_queries >= MAX_ACTIVE_QUERIES {
            bail!(
                "cursor_capacity: core.workspace:fs.list already has {MAX_ACTIVE_QUERIES} active queries"
            );
        }
    ...
    fn publish_reserved(&mut self, state: ListQueryState) -> Result<String> {
        ...
        let usage = self
            ...
            .context("core.workspace:fs.list active-query reservation disappeared")?;
        let retained_entries = usage.retained_entries.checked_add(entry_count).context(
            "cursor_capacity: core.workspace:fs.list retained-entry accounting overflowed",
        )?;
        let retained_bytes = usage
            .retained_bytes
            .checked_add(state.retained_bytes)
            .context(
                "cursor_capacity: core.workspace:fs.list retained-byte accounting overflowed",
            )?;
        if retained_entries > MAX_RETAINED_ENTRIES || retained_bytes > MAX_RETAINED_BYTES {
            bail!(
                "cursor_capacity: core.workspace:fs.list retained-query budget would be exceeded"
            );
        }
    ...
    while let Some(directory) = pending.pop() {
        ...
        for entry in directory_entries.entries {
            ...
            let name = name.to_str().ok_or_else(|| {
                anyhow::anyhow!(
                    "non_utf8_path: core.workspace:fs.list encountered a non-UTF-8 Linux filename"
                )
            })?;
            if name == ".git" {
                continue;
            ...
            let kind = if file_type.is_dir() {
                ...
                bail!(
                    "unsupported_path_type: core.workspace:fs.list cannot represent {}",
                    entry.path().display()
                );
            };
...
fn ensure_directory_unchanged(
    ...
    let actual = directory_fingerprint(tools, path)?;
        ...
        actual == *expected,
        "cursor_stale: core.workspace:fs.list directory changed while it was being scanned"
    );
    Ok(())
}
...
fn finish_scan(
    ...
    if let Err(error) = validate_directories(tools, &directories) {
        bail!(
            "cursor_stale: core.workspace:fs.list traversal changed before publication: {error:#}"
        );
    }
...
fn retained_size(run_id: &str, query: &QueryFingerprint, scanned: &ScanResult) -> Result<usize> {
    let mut total = mem::size_of::<ListQueryState>()
        ...
        .context(
            "cursor_capacity: core.workspace:fs.list retained-query byte accounting overflowed",
        )?;
    for bytes in [
        run_id.len(),
        query.path.capacity(),
        ...
        scanned
            ...
            .context("cursor_capacity: core.workspace:fs.list retained-entry byte accounting overflowed")?,
        scanned
            .directories
            .capacity()
            .checked_mul(mem::size_of::<DirectoryFingerprint>())
            .context("cursor_capacity: core.workspace:fs.list retained-directory byte accounting overflowed")?,
    ] {
        total = total
            .checked_add(bytes)
            .context("cursor_capacity: core.workspace:fs.list retained-query byte accounting overflowed")?;
    }
    for entry in &scanned.entries {
        total = total
            ...
            .context(
                "cursor_capacity: core.workspace:fs.list retained-entry byte accounting overflowed",
            )?;
    }
    for directory in &scanned.directories {
        total = total.checked_add(directory.path.capacity()).context(
            "cursor_capacity: core.workspace:fs.list retained-directory byte accounting overflowed",
        )?;
    }
    Ok(total)
}
...
fn query_deadline(now: u64) -> Result<u64> {
    now.checked_add(QUERY_TTL_MS)
        .context("cursor_capacity: core.workspace:fs.list cursor expiry overflowed")
}
...
fn parse_glob(pattern: &str, case: GlobCase) -> Result<Vec<GlobToken>> {
    ...
    while index < chars.len() {
        match chars[index] {
            ...
            '\0'..='\u{1f}' | '\u{7f}' => {
                bail!("invalid_glob: core.workspace:fs.list glob contains a control character")
            }
            literal => {
...
fn parse_class(chars: &[char], mut index: usize, case: GlobCase) -> Result<(GlobToken, usize)> {
    ...
    while let Some(current) = chars.get(index).copied() {
        if current == ']' && saw_item {
            ...
            current != '/',
            "invalid_glob: core.workspace:fs.list character class cannot contain `/`"
        );
        let end = if chars.get(index + 1) == Some(&'-') {
            let end = chars
                .get(index + 2)
                .copied()
                .context("invalid_glob: core.workspace:fs.list character range has no end")?;
            ensure!(
                end != ']' && end != '/',
                "invalid_glob: core.workspace:fs.list character range is invalid"
            );
            index += 3;
            end
        } else {
            ...
            current <= end,
            "invalid_glob: core.workspace:fs.list character range is reversed"
        );
        ensure!(
            fold_ascii(current, case) <= fold_ascii(end, case),
            "invalid_glob: core.workspace:fs.list character range reverses after ASCII case folding"
        );
        ranges.push((current, end));
        saw_item = true;
    }
    bail!("invalid_glob: core.workspace:fs.list character class is not closed")
}
