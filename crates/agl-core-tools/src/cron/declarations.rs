...
pub fn declaration() -> ExtensionDescriptor {
    ...
    .with_tool(action::<HistoryArgs>(
        CRON_HISTORY_TOOL_ID,
        "Show recorded runs for one cron job.",
        OperationKind::Read,
    ))
    .with_tool(action::<JobDraftArgs>(
    ...
    .with_tool(
        ...
        .with_state_effects([EffectId::store_cron()]),
    )
    .with_tool(
        action::<UpdateArgs>(
            CRON_UPDATE_TOOL_ID,
    ...
    .with_tool(
        ...
        .with_state_effects([EffectId::store_cron(), EffectId::store_idempotency()]),
    )
    .with_tool(
        action::<TickArgs>(
            CRON_TICK_TOOL_ID,
...
fn action<T: JsonSchema>(
    ...
    ToolDeclaration::from_schema::<T>(
        ToolId::new(id).expect("builtin cron action id is valid"),
        description,
        operation_kind,
    )
