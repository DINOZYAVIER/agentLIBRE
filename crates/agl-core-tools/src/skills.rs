...
pub fn declaration() -> ExtensionDescriptor {
    ...
    .with_tool(action::<SkillInspectArgs>(
...
mod tests {
    ...
    fn all_skill_action_schemas_compile_and_reject_unknown_fields() {
        ...
        assert_eq!(declaration.tools.len(), 6);
        ...
        let list = declaration
            .tool(&ToolId::new(SKILL_LIST_TOOL_ID).unwrap())
            .unwrap()
            .compile_schema()
            .unwrap();
    ...
    fn mutating_skill_actions_declare_precise_effects() {
        ...
        assert_eq!(
            ...
            [EffectId::skill_trust()].into_iter().collect()
        );
        assert_eq!(
